...
public class OboIdUtilities {
    ...
    @Nonnull
    public static Optional<OboId> getOboIdFromIri(@Nonnull IRI iri) {
        String iriString = iri.toString();
        Matcher matcher = OBO_ID_IRI_PATTERN.matcher(iriString);
        if(!matcher.find()) {
            return Optional.empty();
        }
        return Optional.of(matcher.group(ID_SPACE_GROUP) + ":" + matcher.group(LOCAL_ID_GROUP)).map(OboId::valueOf);
    }

    @Nonnull
    public static Optional<OboId> getOboIdFromEntity(@Nonnull OWLEntity entity) {
        return getOboIdFromIri(entity.getIRI());
    }
